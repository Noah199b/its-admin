...
function doShowEditDialog(){
	...
	if($(this).hasClass("btn-update")){
		...
		//定义修改时的标题
		title="修改活动,id为"+$("#modal-dialog").data("id");
	}
	//2.启动模态框,并加载页面
	//在模态框对应位置异步加载url
	var url="notice/editUI.do";
...
function doGetObjects(){
	...
	var url="notice/doFindObjects.do"
...
function setTableBodyRows(result){//result 为一个json对象
	...
	tr.append("<td>"+(result[i].state?"启用":"禁用")+"</td>");
